...
namespace ICSP.Core.Reflection
{
  public static class MethodHelper
  {
    ...
    public static (Type Type, string MethodName) GetMethodInfo(this MethodBase method)
    {
      if(method == null)
        return (null, null);

      // <<Main>b__0>d
      var lDeclaringType = method.DeclaringType;
      ...
      if(lMethodName == "MoveNext" || lMethodName.Contains("<"))
      {
        if(lMethodName == "MoveNext")
        {
          ...
          lMethodName = AsychMethodName.Match(lDeclaringType?.Name ?? string.Empty).Groups["Name"].Value;
        ...
        else
        {
          ...
          if(lDeclaringType?.Name.Contains("<") ?? false)
          {
            var lType = method.DeclaringType;
    ...
    public static (string Type, string MethodName) GetMethodName(this MethodBase method)
    {
      if(method == null)
        return (null, null);
